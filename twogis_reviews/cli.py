...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="2GIS reviews parser")

    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--url", type=str, help="Direct URL to organization page")
    source.add_argument("--firm-id", type=str, help="Firm ID (e.g., 70000001019342551)")
    source.add_argument("--input-file", type=str, help="Path to text file with URLs or IDs (one per line)")

    parser.add_argument("--city", type=str, default="moscow", help="City slug (default: moscow)")
...
async def run_single(parser: TwoGisParser, target: str, city: str) -> ParseResult:
    if target.startswith("http"):
        return await parser.parse_by_url(target)
    return await parser.parse_by_firm_id(target, city)


async def run_batch(parser: TwoGisParser, input_file: str, city: str) -> list[ParseResult]:
...
def save_single_result(result: ParseResult, output_path: str | None) -> str:
    if not output_path:
        ...
        output_path = f"twogis_reviews_{timestamp}.json"
...
def save_batch_results(results: list[ParseResult], output_path: str | None) -> str:
    if not output_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"twogis_batch_{timestamp}.json"
...
def main():
    ...
    try:
        if args.input_file:
            ...
            target = args.url or args.firm_id
