...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Yandex Maps reviews parser")

    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--url", type=str, help="Direct URL to organization page")
    source.add_argument("--org-id", type=str, help="Organization ID")
...
async def run_single(parser: YandexReviewsParser, target: str) -> ParseResult:
    if target.startswith("http"):
        return await parser.parse_by_url(target)
    return await parser.parse_by_org_id(target)


async def run_batch(parser: YandexReviewsParser, input_file: str) -> list[ParseResult]:
...
def save_batch_results(results: list[ParseResult], output_path: str | None) -> str:
    if not output_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"reviews_batch_{timestamp}.json"
...
def main():
    ...
    parser = YandexReviewsParser(**kwargs)
    ...
    try:
        if args.input_file:
            ...
            target = args.url or args.org_id
