...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Google Maps reviews parser")

    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--url", type=str, help="Direct URL to Google Maps place")
    source.add_argument("--place-id", type=str, help="Google place_id")
    source.add_argument("--input-file", type=str, help="Path to text file with URLs or place IDs (one per line)")
    ...
    parser.add_argument("--scroll-pause", type=float, help="Scroll pause (seconds)")
    parser.add_argument("--scroll-step", type=int, help="Scroll step (pixels)")
    parser.add_argument("--timeout", type=int, help="Page load timeout (seconds)")
    parser.add_argument("--initial-delay", type=int, help="Initial delay before scrolling (ms)")
...
def get_parser_kwargs(args: argparse.Namespace) -> dict:
    ...
    if args.scroll_pause is not None:
        kwargs["scroll_pause_sec"] = args.scroll_pause
    if args.scroll_step is not None:
        kwargs["scroll_step_px"] = args.scroll_step
...
async def run_single(parser: GoogleReviewsParser, target: str) -> ParseResult:
    if target.startswith("http"):
        return await parser.parse_by_url(target)
    return await parser.parse_by_place_id(target)


async def run_batch(parser: GoogleReviewsParser, input_file: str) -> list[ParseResult]:
    ...
    for idx, target in enumerate(targets, 1):
        ...
        try:
            res = await run_single(parser, target)
            results.append(res)
...
def save_single_result(result: ParseResult, output_path: str | None) -> str:
    if not output_path:
        ...
        output_path = f"google_reviews_{timestamp}.json"
...
def save_batch_results(results: list[ParseResult], output_path: str | None) -> str:
    if not output_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"google_reviews_batch_{timestamp}.json"
...
def main():
    ...
    parser = GoogleReviewsParser(**kwargs)

    try:
        if args.input_file:
            results = asyncio.run(run_batch(parser, args.input_file))
            if args.save_db:
            ...
            target = args.url or args.place_id
            result = asyncio.run(run_single(parser, target))
            if args.save_db:
