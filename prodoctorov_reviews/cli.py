...
from .parser import ProdoctorovParser
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ПроДокторов reviews parser")

    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--url", type=str, help="Direct URL to clinic/reviews page")
    source.add_argument("--lpu-id", type=str, help="Clinic LPU ID (e.g., 69674-medpraym)")
    source.add_argument("--input-file", type=str, help="Path to text file with URLs or IDs (one per line)")

    parser.add_argument("--city", type=str, default="moskva", help="City slug (default: moskva)")
...
async def run_single(parser: ProdoctorovParser, target: str, city: str) -> ParseResult:
    if target.startswith("http"):
        return await parser.parse_by_url(target)
    return await parser.parse_by_lpu_id(city, target)


async def run_batch(parser: ProdoctorovParser, input_file: str, city: str) -> list[ParseResult]:
...
def save_single_result(result: ParseResult, output_path: str | None) -> str:
    if not output_path:
        ...
        output_path = f"prodoctorov_reviews_{timestamp}.json"
...
def save_batch_results(results: list[ParseResult], output_path: str | None) -> str:
    if not output_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"prodoctorov_batch_{timestamp}.json"
...
def main():
    ...
    parser = ProdoctorovParser(**kwargs)
    ...
    try:
        if args.input_file:
            ...
            target = args.url or args.lpu_id
